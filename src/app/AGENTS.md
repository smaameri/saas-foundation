...
- `src/app/(admin)/admin`: authenticated experience. Layout fetches the Better Auth session via `fetchSession()` and redirects to `/login` when `session.session` is missing. Add new protected pages (e.g., `/dashboard`) inside this folder.
...
All requests to the backend must go through an API service in `src/lib/api/`. Never call `fetch` directly in components — use the appropriate API service method instead.

API services are scoped by portal and loosely structured by entity:

- `src/services/api/client.ts` — base `ApiClient` class with `get`, `post`, `patch`, `put`, `delete` methods. Throws `ApiError` on non-2xx responses.
